...
const SCHEMA_STATEMENTS = [
  ...
  // Market Archive Hardening -- MLB/NFL/NCAAF provider-line archives.
  // Identical schema/discipline to nba_provider_line_archive above (built
  // via lib/marketArchive/providerLineArchiveFactory.js rather than a
  // third/fourth/fifth hand-copied module); entirely separate tables,
  // never merged with NBA/WNBA or with each other. Fixes the Market
  // Intelligence audit's Gap 2 (MLB/NFL/NCAAF previously had no genuine
  // provider-line time-series archive at all).
  `CREATE TABLE IF NOT EXISTS mlb_provider_line_archive (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    captured_at INTEGER NOT NULL,
    provider_last_update TEXT,
    age_seconds REAL,
  ...
  `CREATE INDEX IF NOT EXISTS idx_mpla_identity ON mlb_provider_line_archive(sport, event_id, player_raw, market_key_raw, source, projection_type, period)`,
  `CREATE INDEX IF NOT EXISTS idx_mpla_event ON mlb_provider_line_archive(event_id)`,
  `CREATE INDEX IF NOT EXISTS idx_mpla_captured ON mlb_provider_line_archive(captured_at)`,
  `CREATE INDEX IF NOT EXISTS idx_mpla_source ON mlb_provider_line_archive(source)`,
  `CREATE INDEX IF NOT EXISTS idx_mpla_market ON mlb_provider_line_archive(market_key_raw)`,
  `CREATE INDEX IF NOT EXISTS idx_mpla_player ON mlb_provider_line_archive(player_raw, market_key_raw)`,

  `CREATE TABLE IF NOT EXISTS nfl_provider_line_archive (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    captured_at INTEGER NOT NULL,
    provider_last_update TEXT,
    age_seconds REAL,
  ...
  `CREATE INDEX IF NOT EXISTS idx_fpla_identity ON nfl_provider_line_archive(sport, event_id, player_raw, market_key_raw, source, projection_type, period)`,
  `CREATE INDEX IF NOT EXISTS idx_fpla_event ON nfl_provider_line_archive(event_id)`,
  `CREATE INDEX IF NOT EXISTS idx_fpla_captured ON nfl_provider_line_archive(captured_at)`,
  `CREATE INDEX IF NOT EXISTS idx_fpla_source ON nfl_provider_line_archive(source)`,
  `CREATE INDEX IF NOT EXISTS idx_fpla_market ON nfl_provider_line_archive(market_key_raw)`,
  `CREATE INDEX IF NOT EXISTS idx_fpla_player ON nfl_provider_line_archive(player_raw, market_key_raw)`,

  `CREATE TABLE IF NOT EXISTS ncaaf_provider_line_archive (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    captured_at INTEGER NOT NULL,
    provider_last_update TEXT,
    age_seconds REAL,
  ...
  `CREATE INDEX IF NOT EXISTS idx_cpla_identity ON ncaaf_provider_line_archive(sport, event_id, player_raw, market_key_raw, source, projection_type, period)`,
  `CREATE INDEX IF NOT EXISTS idx_cpla_event ON ncaaf_provider_line_archive(event_id)`,
